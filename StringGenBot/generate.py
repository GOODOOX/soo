...
async def generate_session(bot: Client, msg: Message, telethon=False, is_bot: bool = False):
    ...
    if not is_bot:
        t = "» ✔️الان ارسل رقمك مع رمز دولتك , مثال :+201447542064''"
    else:
    ...
    try:
        phone_code_msg = None
        if not is_bot:
            phone_code_msg = await bot.ask(user_id, "[ارسل الكود زي اللي في الصوره](https://telegra.ph/file/da1af082c6b754959ab47.jpg)» 🔍جالك كود من بوت شركه تليجرام \n  هناك تحقق بخطوتين( المرور ) ، أرسل كلمة المرور للبوت بعد ارسال كود الدخول بنفس الطريقه اللي في الصوره.- ابعتو للبوت عشان يستخرج الجلسه \n 12345 يرجى ارسالها بالشكل التالي 1 2 3 4 5 مع وجود مسـافـات بين الارقام اذا احتجت مساعدة @VIP_ALPOP.\n وانضم في قناه السورس بنزل شرح @SOURCE_ALPOP \n وانضم في جروب الدعم @baralpop", filters=filters.text, timeout=600)
    ...
    text = f"**جلستك يبروو {ty} sᴛʀɪɴɢ sᴇssɪᴏɴ** \n\n`{string_session}` \n\n**𝗗𝗘𝗩 𝗕𝗢𝗧 :** https://t.me/VIP_ALPOP \n🍒 **هام جدا :** حافظ عليها ممكن حد يخترقكك بيها\n انضم هناا قناه السورس بنزل شرح وانضم في جروب الدعم @baralpop @SOURCE_ALPOP 🥺"
    try:
        if not is_bot:
